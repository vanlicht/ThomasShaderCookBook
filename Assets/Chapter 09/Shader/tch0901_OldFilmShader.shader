Shader "Custom/tch0901_OldFilmShader" 
{
	Properties {
		_MainTex("Base (RGB)", 2D) = "white" {}
		_VignettetTex("Vignette Texture", 2D) = "white"{}
		...
		_ScrathesXSpeed("Scratches X Speed", Float) = 10.0
	...
	SubShader
	{
		Pass
		{
			...
			fixed4 frag(v2f_img i) : COLOR
			{
				...
				distortedUV = half2(i.uv.x, i.uv.y + (_RandomValue * _SinTime.z * 0.0001));
				fixed4 renderTex = tex2D(_MainTex, i.uv);
				fixed4 vignetteTex = tex2D(_VignetteTex, i.uv);

				//Process the scratches UV and pixels
				half2 scratchesUV =
					half2(i.uv.x + (_RandomValue * _SinTime.z * _ScratchesXSpeed),
						i.uv.y + (_RandomValue * _SinTime.z * _ScratchesYSpeed));
				fixed4 scratchesTex = tex2D(_ScratchesTex, scratchesUV);

				//Process Dust UV and pixels
				half2 dustUV =
					half2(i.uv.x + (_RandomValue * _SinTime.z * _dustXSpeed),
				...
				//Creat a constant white color we can use to adjust opacity of effects
				fixed3 constantWhite = fixed3(1, 1, 1);

				//Compositi together the different layers to create final screen effect
